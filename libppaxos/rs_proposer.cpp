/**
 * rs_proposer.cpp
 * Created on Semptember 20, 2015
 * Author: Lijing Wang
 */

#include "rs_proposer.hpp"
#include <iostream>
namespace ppaxos {
...
//  std::cout << "Proposer handle_msg_promise from node_id: " << node_id << std::endl;
  ...
  // NOT_ENOUGH
  if (msg_ack_prepare_.size() <= view_->nodes_size() / 2) {
...
//    std::cout << "\tProposer promise ---- NOT_ENOUGH!" << std::endl;
  ...
  if (true_counter > view_->nodes_size() / 2) {
    // CONTINUE
...
//  std::cout << "Inside handle_msg_accepted node_id: " << node_id << std::endl;
  ...
  // NOT_ENOUGH
  if (msg_ack_accept_.size() <= view_->nodes_size() / 2) {
